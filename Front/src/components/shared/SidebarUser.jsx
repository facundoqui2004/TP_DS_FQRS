...
const Sidebar = (props) => {
  ...
  return (
    <div
      className={`bg-[#1F1D2B] fixed lg:left-0 top-0 w-28 h-screen max-h-screen flex flex-col justify-between rounded-tr-xl rounded-br-xl z-50 transition-all duration-300 ease-in-out shadow-lg overflow-x-hidden
      ${showMenu ? 'left-0' : '-left-full'}`}
      style={{
        scrollbarWidth: "thin",
        scrollbarColor: "#ec7c6a #1F1D2B",
      }}
    >
      {/* Logo */}
      ...
      <div className="flex-1 overflow-y-auto py-2 scrollbar-thin overflow-x-hidden w-full">
        <ul className="pl-4 space-y-1 w-full overflow-x-hidden">
          <li className="hover:bg-[#262837] p-1.5 block rounded-tl-xl rounded-bl-xl group transition-colors">
            <button
              onClick={() => navigate("/")}
              className="bg-[#ec7c6a] p-4 flex justify-center w-full rounded-xl text-white relative cursor-pointer"
            >
            ...
            <div className="relative">
              {isAuthenticated ? (
                <button
                  ...
                  className="group-hover:bg-[#ec7c6a] p-4 flex justify-center items-center w-full rounded-xl text-[#ec7c6a] group-hover:text-white cursor-pointer"
                >
