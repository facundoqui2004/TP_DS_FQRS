...
export const getPoderes = (req, res) => {
    res.json({ data: poderes });
};
//Obtener un poder por id
export const getPoder = (req, res) => {
...
//Crear un poder
export const createPoder = (req, res) => {
    ...
    // Instancia nueva
    const nuevo = new Poder(undefined, req.body.nom_poder, req.body.debilidad, req.body.desc_poder, req.body.desc_debilidad, req.body.categoria, req.body.costoMulta);
    poderes.push(nuevo);
    res.status(201).json({ message: "Poder creado", data: nuevo });
};
// Actualizar un poder
export const updatePoder = (req, res) => {
    const id = req.params.id;
...
// Eliminar un poder
export const deletePoder = (req, res) => {
    const id = req.params.id;
    const index = poderes.findIndex(p => p.id_Poder === id);
    if (index === -1) {
        return res.status(404).json({ error: "No encontrado" });
...
//# sourceMappingURL=poder.controller.js.map